...
- Quick ZIP installer (no external tools required):
  - Build artifacts (on Windows machine):
    ...
    ```
  - This creates `dist\mercury-framework-0.1.0-win.zip` and `dist\install.ps1`.
...
- MSI installer (requires WiX Toolset):
  ...
  - Build MSI:
    ...
    ```
  - Result: `dist\mercury-framework-0.1.0.msi`.
...
- Build `.pkg` and `.dmg` on macOS:
  ```bash
  ./scripts/build_macos_installer.sh 0.1.0
  ```
- Result: `dist/mercury-framework-0.1.0.pkg` and `dist/mercury-framework-0.1.0-mac.dmg`.
...
- Run on a Debian/Ubuntu host with `dpkg-deb` and `fakeroot` installed:
  ...
  ./scripts/build_debian.sh 0.1.0
  ```
- Result: `dist/mercury-framework_0.1.0_all.deb`.

Arch Linux
----------
- Prepare PKGBUILD and optional `makepkg` build on Arch:
  ```bash
  ./scripts/build_arch.sh 0.1.0        # prepares PKGBUILD and source tarball
  ./scripts/build_arch.sh 0.1.0 --makepkg  # builds the package (on Arch)
  ```
...
- Quick start after extraction (Python 3.8+):
  ```powershell
  python -m venv .venv
...
Notes
-----
- These installer scripts intentionally do not bundle Python. Ensure Python 3.8+ is available on target systems.
